...
<x-NavbarAdmin/>
    ...
    <section class=" mt-10 grid grid-cols-1 text-center justify-center align-middle">
        ...
        <form method="POST" action="{{ route("Updatesolution" , $solutionsdata->id ) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="text" placeholder="Title"
                class="input input-bordered input-primary w-full max-w-xs mb-5 mt-5 " name="Title" value="{{ $solutionsdata->Title }}" />
            <br>

            <input type="text" placeholder="description"
                class="input input-bordered input-primary w-full max-w-xs " name="description" value="{{ $solutionsdata->description }}"/>
