...
<body>
    ...
    {{-- <form method="POST" action="{{ route('storeSetting') }}" enctype="multipart/form-data">
      ...
      <input type="text" placeholder="Project Title"
          class="input input-bordered input-primary w-full max-w-xs mb-5 mt-5 " name="settingType" />
      <br>

      <input type="text" placeholder="Project description"
          class="input input-bordered input-primary w-full max-w-xs " name="data" />
    ...
    <section class="mt-10 mb-11 xl:p-14 ">
        <div class="overflow-x-scroll">
            <table class="table text-4xl border border-gray-600">
                ...
                <thead>
                    <tr class=" ">
                        <th class="text-xl font-bold ">Setting</th>

                        <th class="text-xl font-bold">Data</th>
                ...
                <tbody>
                    @foreach ($Settingsdata as $setting)
                        <tr>
                            ...
                            @if ($setting->logo)
                            <td class="border border-gray-600">{{ $setting->settingType }}</td>
                            <td class="border border-gray-600"><img style="width: 50px ; height:50px;" src="{{ asset($setting->logo) }}" alt="" srcset=""></td>
                            @else 
                            <td class="border border-gray-600">{{ $setting->settingType }}</td>
                            <td class="border border-gray-600">{{ $setting->data }}</td>
                            @endif


                            <td class="border border-gray-600">
                                <a href="{{ route('editSetting', $setting->id) }}" class="btn btn-primary">
